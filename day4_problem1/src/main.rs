...
fn main() {
    ...
    let total_sleep_by_guard = guards_sleep_intervals.iter().fold(HashMap::new(), |mut acc, interval| {
        *acc.entry(interval.id).or_insert(0) += interval.end-interval.start; return acc;});
    
    let guard_with_most_sleep = total_sleep_by_guard.into_iter().max_by_key(|a| a.1).unwrap().0;
    let sleep_intervals_for_guard_with_most_sleep = get_sleep_intervals_for_guard(guard_with_most_sleep, &guards_sleep_intervals);
    let most_likely_minute_asleep = get_most_likely_minute_asleep(sleep_intervals_for_guard_with_most_sleep).0;    

    println!("Result = {}", guard_with_most_sleep * most_likely_minute_asleep);
